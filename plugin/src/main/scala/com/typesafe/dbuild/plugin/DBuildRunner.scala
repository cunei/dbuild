...
object DBuildRunner {
  ...
  def newState(state: State, extracted: Extracted, update: Seq[Setting[_]] => Seq[Setting[_]], log: Logger) = {
    import extracted._
    ...
    val newStructure = reapplySettings(oldSettings ++ newSettings, structure, log) // ( Project.showContextKey(newSession, structure) )
  ...
  def rewire(state: State, previousOnLoad: State => State, fixPublishSettings: Boolean = false): State = {
    ...
    saveMsgDef(newState(_, extracted, newSettings, log))(state)
  }
